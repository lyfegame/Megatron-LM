...
def get_v32_indexer_mapping_list() -> list:
    ...
    - wq_b: Projects compressed Q to indexer heads (column-parallel)
    ...
    indexer_mappings = {
        ...
        "decoder.layers.*.self_attention.lightning_indexer.linear_wq_b.weight": "model.layers.*.self_attn.indexer.wq_b.weight",
        # wk: hidden_size -> head_dim (replicated)
        "decoder.layers.*.self_attention.lightning_indexer.linear_wk.weight": "model.layers.*.self_attn.indexer.wk.weight",
        # k_norm: LayerNorm for K (replicated)
        "decoder.layers.*.self_attention.lightning_indexer.k_layernorm.weight": "model.layers.*.self_attn.indexer.k_norm.weight",
        "decoder.layers.*.self_attention.lightning_indexer.k_layernorm.bias": "model.layers.*.self_attn.indexer.k_norm.bias",
        # weights_proj: per-head aggregation weights
        "decoder.layers.*.self_attention.lightning_indexer.linear_weights_proj.weight": "model.layers.*.self_attn.indexer.weights_proj.weight",
    }

    mapping_list = []
    for megatron_param, hf_param in indexer_mappings.items():
